...
interface TextEditorProps {
  ...
  onSave: (data: { content: Record<string, string>; ratings: RatingField[] }) => void;
}

const TextEditor: React.FC<TextEditorProps> = ({ userData, onSave }) => {
  const [content, setContent] = useState<Record<string, string>>(userData.content || DEFAULT_CONTENT);
  const [ratings, setRatings] = useState<RatingField[]>(userData.ratings || DEFAULT_RATINGS);
  ...
  // Автосохранение при изменении (можно добавить debounce)
  useEffect(() => {
    onSave({ content, ratings });
  }, [content, ratings]);
    ...
    <div className="w-full h-screen bg-editor-bg overflow-hidden relative">
      ...
      <div className="h-[60px] border-b border-editor-separator flex items-center justify-between px-4 bg-black bg-opacity-90 backdrop-blur-md z-10">
        ...
        <div className="flex space-x-4">
          <button onClick={handleDownload} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors">
            Скачать оценки
          </button>
          <Dialog open={isGuideOpen} onOpenChange={setIsGuideOpen}>
            ...
            <DialogContent className="bg-black text-green-500 border border-green-700 p-4 rounded-lg transition-all duration-300">
              <div className="mt-4 text-sm text-center">
                ...
                <br /><br />
                При наведении на область оценки в рейтинге появятся кнопки управления:
                редактирование, удаление и добавление пояснения.
                <br /><br />
                Также снизу есть две области: "ПЛЮСЫ" и "МИНУСЫ".
                <br /><br />
                <span className="text-xs">Все данные сохраняются на сервере.</span>
              </div>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div className="p-6 overflow-y-auto" style={{ height: 'calc(100vh - 60px)' }}>
        <div className="max-w-4xl mx-auto">
          ...
          {/* Текстовые поля (ПЛЮСЫ и МИНУСЫ) */}
          {DEFAULT_FIELDS.filter(field => !field.isRating).map((field) => (
            <div
              key={field.id}
              className="mb-8 animate-text-focus-in"
              style={{ animationDelay: `${DEFAULT_FIELDS.indexOf(field) * 0.1}s` }}
            >
